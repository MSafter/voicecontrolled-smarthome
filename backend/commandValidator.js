...
exports.validateCommand = function(req, res) {
  ...
  if (commandInformation) {
    switch (commandInformation.command.type) {
      case 'node':
        ...
        module[commandInformation.command.functionname](function(err, data) {
          if (err) {
            return res.send({
              message: 'Error executing command ' + commandInformation.phrase
            });
          }
    ...
    return res.send({
      message: 'No command configured for phrase' + userPhrase
    });
  }
};
...
function getCommandInformation(phrase) {
  ...
  let keyPhrases = config.commands.map(item => {
    return item.keyphrase.toLowerCase();
  });
  ...
  config.commands.forEach((item, index) => {
    item.commands.forEach((command, comIndex) => {
      ...
      keyWords.forEach(keyWord => {
        ...
        if (
          currentCommandPhrase.trim().toLowerCase() ===
          compareCommandPhrase.trim().toLowerCase()
        ) {
